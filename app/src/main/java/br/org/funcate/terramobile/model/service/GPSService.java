...
public class GPSService {
    ...
    public static Location getLastKnownLocation(Context context) {
        return GPSService.getLocationManager(context).getLastKnownLocation(LocationManager.GPS_PROVIDER);
    }
    ...
    public static boolean registerListener(Context context, LocationListener locationListener) {
        return registerListener(context, locationListener, GPSService.MIN_TIME_BW_UPDATES, GPSService.MIN_DISTANCE_CHANGE_FOR_UPDATES);
    }

    public static boolean registerListener(Context context, LocationListener locationListener, long updateTime) {
        return registerListener(context, locationListener, updateTime, GPSService.MIN_DISTANCE_CHANGE_FOR_UPDATES);
    }

    public static boolean registerListener(Context context, LocationListener locationListener, long updateTime, long distanceForUpdate) {
        if (GPSService.isGPSProviderEnable(context)) {
            GPSService.getLocationManager(context).requestLocationUpdates(
                    ...
                    locationListener);
            return true;
        }else if(GPSService.isNetworkProviderEnable(context)) {
            GPSService.getLocationManager(context).requestLocationUpdates(
